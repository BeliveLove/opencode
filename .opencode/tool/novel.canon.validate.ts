...
import DESCRIPTION from "./novel.canon.validate.txt"
...
export default tool({
  ...
  args: {
    chapterId: tool.schema.string().optional().describe("Optional chapter id to validate refs (e.g. CH_01_003)"),
  },
  async execute(args) {
    const issues: Issue[] = []
    ...
    if (!exists) {
      ...
      return YAML.stringify({ ok: false, issues }).trimEnd()
    }
    ...
    for (const kind of CANON_KINDS) {
      ...
      for (const item of all[kind]) {
        ...
        if (prefix && !item.id.startsWith(prefix)) {
          issues.push({ level: "error", code: "BAD_ID_PREFIX", kind, id: item.id, message: `id 前缀不合法：期望 ${prefix}*，实际 ${item.id}` })
        }
        ...
        if (kind === CanonKind.rules && (!item.name || !item.rule)) {
          issues.push({ level: "warning", code: "MISSING_RULE_FIELDS", kind, id: item.id, message: "设定建议至少包含 name 与 rule" })
        }
        ...
        if (kind === CanonKind.foreshadow) {
          ...
          if (!item.promise) issues.push({ level: "warning", code: "MISSING_PROMISE", kind, id: item.id, message: "伏笔缺少 promise" })

          const payoffChapter = item.payoff?.chapter
          if (status === "paid_off" && !payoffChapter) {
            issues.push({ level: "error", code: "PAIDOFF_MISSING_PAYOFF", kind, id: item.id, message: "paid_off 必须填写 payoff.chapter" })
          }
          if (status !== "paid_off" && payoffChapter) {
            issues.push({ level: "warning", code: "PAYOFF_SET_BUT_NOT_PAID", kind, id: item.id, message: "payoff.chapter 已填写但 status 不是 paid_off" })
          }
        }
      }
    }
    ...
    if (args.chapterId) {
      ...
      if (!chapterContent.trim()) {
        issues.push({ level: "error", code: "CHAPTER_NOT_FOUND", chapterId: args.chapterId, message: `未找到章节文件：novel/chapters/${args.chapterId}.md` })
      } else {
        ...
        for (const id of refs) {
          if (!allIds.has(id)) {
            issues.push({ level: "error", code: "MISSING_REF", id, chapterId: args.chapterId, message: `引用不存在（请写回 canon/）：${id}` })
          }
        }

        const fm = parsed.frontmatter
        if (fm.id && fm.id !== args.chapterId) {
          issues.push({ level: "warning", code: "CHAPTER_ID_MISMATCH", chapterId: args.chapterId, message: `frontmatter.id 与文件名不一致：${fm.id} vs ${args.chapterId}` })
        }
        if (fm.participants && !isStringArray(fm.participants)) {
          issues.push({ level: "warning", code: "BAD_PARTICIPANTS", chapterId: args.chapterId, message: "frontmatter.participants 应为 string[]" })
        }
      }
    }

    const ok = issues.every((x) => x.level !== "error")
    return YAML.stringify({ ok, issues }).trimEnd()
  },
})
