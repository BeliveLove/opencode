...
function use() {
  ...
  if (!ctx) throw new Error("useContext 必须在 Session 组件中使用")
  return ctx
}
    ...
    },
    {
      ...
      onSelect: (dialog) => {
        ...
        if (textParts.length === 0) {
          toast.show({ message: "上一条助手消息中未找到文本部分", variant: "error" })
          dialog.clear()
          return
        }
  ...
  const revertDiffFiles = createMemo(() => {
    ...
    try {
      ...
      return patches.map((patch) => {
        const filename = patch.newFileName || patch.oldFileName || "未知"
                          ...
                          >
                            ...
                            >
                              ...
                              <text fg={theme.textMuted}>
                                <span style={{ fg: theme.text }}>{keybind.print("messages_redo")}</span> 或使用 /redo
                                恢复
                              </text>
      ...
      <Show when={text()}>
        ...
        >
          ...
          >
            ...
            >
              <text fg={theme.textMuted}>
                <span style={{ bg: theme.accent, fg: theme.backgroundPanel, bold: true }}> 排队中 </span>
              </text>
            </Show>
          </box>
        </box>
...
function GenericTool(props: ToolProps<any>) {
  return (
    <InlineTool icon="⚙" pending="正在编写命令..." complete={true} part={props.part}>
...
function Bash(props: ToolProps<typeof BashTool>) {
  ...
  const title = createMemo(() => {
    const desc = props.input.description ?? "终端"
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="$" pending="正在编写命令..." complete={props.input.command} part={props.part}>
          {props.input.command}
        </InlineTool>
      </Match>
    </Switch>
  ...
  return (
    <Switch>
      <Match when={props.metadata.diagnostics !== undefined}>
        <BlockTool title={"# 已写入 " + normalizePath(props.input.filePath!)} part={props.part}>
...
function Glob(props: ToolProps<typeof GlobTool>) {
  return (
    <InlineTool icon="✱" pending="查找文件..." complete={props.input.pattern} part={props.part}>
      匹配 "{props.input.pattern}" <Show when={props.input.path}>在 {normalizePath(props.input.path)} </Show>
      <Show when={props.metadata.count}>({props.metadata.count} 条匹配)</Show>
    </InlineTool>
  )
}
...
function Read(props: ToolProps<typeof ReadTool>) {
  return (
    <InlineTool icon="→" pending="读取文件..." complete={props.input.filePath} part={props.part}>
...
function Grep(props: ToolProps<typeof GrepTool>) {
  return (
    <InlineTool icon="✱" pending="搜索内容..." complete={props.input.pattern} part={props.part}>
      搜索 "{props.input.pattern}" <Show when={props.input.path}>在 {normalizePath(props.input.path)} </Show>
      <Show when={props.metadata.matches}>({props.metadata.matches} 条匹配)</Show>
    </InlineTool>
  )
}
...
function List(props: ToolProps<typeof ListTool>) {
  ...
  return (
    <InlineTool icon="→" pending="列出目录..." complete={props.input.path !== undefined} part={props.part}>
      列出 {dir()}
    </InlineTool>
  )
}
...
function WebFetch(props: ToolProps<typeof WebFetchTool>) {
  return (
    <InlineTool icon="%" pending="从网页获取..." complete={(props.input as any).url} part={props.part}>
...
function CodeSearch(props: ToolProps<any>) {
  ...
  return (
    <InlineTool icon="◇" pending="搜索代码..." complete={input.query} part={props.part}>
      Exa 代码搜索 "{input.query}" <Show when={metadata.results}>({metadata.results} 条结果)</Show>
    </InlineTool>
  )
}
...
function WebSearch(props: ToolProps<any>) {
  ...
  return (
    <InlineTool icon="◈" pending="搜索网页..." complete={input.query} part={props.part}>
      Exa 网页搜索 "{input.query}" <Show when={metadata.numResults}>({metadata.numResults} 条结果)</Show>
    </InlineTool>
  )
}
    ...
    <Switch>
        ...
        >
          <box>
            <text style={{ fg: theme.textMuted }}>
              {props.input.description} ({props.metadata.summary?.length} 次工具调用)
            </text>
            <Show when={current()}>
      ...
      <Match when={true}>
        <InlineTool
          ...
          part={props.part}
        >
          <span style={{ fg: theme.text }}>{typeLabel()}</span> 任务 "
...
function Edit(props: ToolProps<typeof EditTool>) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="←" pending="准备编辑..." complete={props.input.filePath} part={props.part}>
  ...
  return (
    <Switch>
      <Match when={props.output !== undefined}>
        <BlockTool title="# 补丁" part={props.part}>
          <box>
      ...
      <Match when={true}>
        <InlineTool icon="%" pending="准备补丁..." complete={false} part={props.part}>
          补丁
        </InlineTool>
      </Match>
    </Switch>
...
function TodoWrite(props: ToolProps<typeof TodoWriteTool>) {
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="⚙" pending="更新待办..." complete={false} part={props.part}>
          更新待办...
        </InlineTool>
      </Match>
    </Switch>
