          ...
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={"这将在 OpenCode 重启前允许 " + props.request.permission + "。"} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>这将在 OpenCode 重启前允许以下模式</text>
                  <box>
              ...
              body={
                <Switch>
                  ...
                  <Match when={props.request.permission === "glob"}>
                    <TextBody icon="✱" title={`匹配 "` + (input().pattern ?? "") + `"`} />
                  </Match>
                  <Match when={props.request.permission === "grep"}>
                    <TextBody icon="✱" title={`搜索 "` + (input().pattern ?? "") + `"`} />
                  </Match>
                  ...
                  <Match when={props.request.permission === "task"}>
                    <TextBody
                      icon="#"
                      title={`${displayAgentName((input().subagent_type as string) ?? "未知")} 任务`}
                      description={"◉ " + input().description}
                    />
                  </Match>
                  <Match when={props.request.permission === "webfetch"}>
                    <TextBody icon="%" title={`网页抓取 ` + (input().url ?? "")} />
                  </Match>
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>告诉 OpenCode 需要如何调整</text>
        </box>
      </box>
      <box
        flexDirection="row"
