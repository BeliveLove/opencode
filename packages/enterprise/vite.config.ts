...
function normalizeWindowsAbsoluteImports(): PluginOption {
  return {
    ...
    transform(code) {
      if (process.platform !== "win32") return
      if (!code.includes(":\\") && !code.includes("from \"") && !code.includes("from '") && !code.includes("import \"") && !code.includes("import '")) {
        return
      }
...
function normalizeWindowsDefineStrings(): PluginOption {
  return {
    ...
    config(config) {
      ...
      const define = config.define as Record<string, unknown> | undefined
      normalize(define)

      const esbuildDefine = (config.esbuild as any)?.define as Record<string, unknown> | undefined
      normalize(esbuildDefine)
    },
  }
}
